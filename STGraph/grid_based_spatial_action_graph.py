...
if args.msvd:
    ...
    save_file = path_to_saved_models+'/grid_spatio_graph.hdf5'
...
FILE_GRID = None
if args.msvd:
    FILE_GRID = "/media02/lnthanh01/phatkhoa/STGraph/extracted/msvd/grid_node_features.hdf5"
else: 
    FILE_GRID = "/media02/lnthanh01/phatkhoa/STGraph/extracted/msrvtt_grid/grid_node_features.hdf5"
    
sample_fp = 1
NUM_PATCHES = 9
frame_num = 16
save_file = "/media02/lnthanh01/phatkhoa/STGraph/extracted/msrvtt_grid/grid_spatio_graph.hdf5"
with torch.no_grad():
    with h5py.File(FILE_GRID, 'r') as fg, h5py.File(save_file, 'w') as f:
        for name in tqdm(name_list):
            ...
            if(len(sample_indx)<args.max_frames):
                additional = args.max_frames - len(sample_indx)
