...
class MSVD_Caption_DataLoader(Dataset):
    ...
    def __init__(
        ...
        self.mapping_names = []
        self.mapping_vid_ids = []
        mapping_file = os.path.join(self.data_path, "captions/youtube-mapping.txt")
        with open(mapping_file, 'r') as f:
            for value in f.readlines():
                name, vid_id = value.split()
                self.mapping_names.append(name)
                self.mapping_vid_ids.append(vid_id)
        ...
        split_dict["train"] = os.path.join(self.data_path, "train_list.txt")
        split_dict["val"] = os.path.join(self.data_path, "val_list.txt")
        split_dict["test"] = os.path.join(self.data_path, "test_list.txt")
    ...
    def _get_video(self, choice_video_ids):
        ...
        video_ids = []
        for vid_name in choice_video_ids:
          video_ids.append(self.mapping_vid_ids[self.mapping_names.index(vid_name)])
        choice_video_ids = video_ids
        
        video_mask = np.zeros((len(choice_video_ids), self.max_frames), dtype=np.int64)

        max_video_length = [0] * len(choice_video_ids)

        video = np.zeros((len(choice_video_ids), self.max_frames, self.feature_size), dtype=np.float64)
        for i, video_id in enumerate(choice_video_ids):
    ...
    def __getitem__(self, idx):
        ...
        video_id = self.mapping_vid_ids[self.mapping_names.index(video_id)]
    ...
    def __generate_object_features(self, fo_path, stgraph_path):
        ...
        for key in chosen_keys:
            fo_input[key] = sparse.csr_matrix(np.array(fo_file.get(key)).astype(np.float64))
